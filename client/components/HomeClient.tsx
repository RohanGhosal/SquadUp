...
export default function HomeClient() {
  ...
  const handleNeuralSearch = async () => {
    ...
    try {
      const queryEmbedding = await getAiManager().getEmbeddings(searchQuery);
      ...
      const scoredLobbies = await Promise.all(lobbies.map(async (lobby) => {
        ...
        const lobbyEmbedding = await getAiManager().getEmbeddings(lobbyText);

        const score = getAiManager().cosineSimilarity(queryEmbedding, lobbyEmbedding);
        return { ...lobby, score };
      }));
